...
public class TestCase {
    ...
    public void testFindVacancyByPhrase() {
        ...
        System.out.println("Поиск по всем полям вакансий, содержащих фразу " + findText.toUpperCase() + ". Найдено " + vacanciesDto.getFound());
        boolean isContainsText=false;
        for(Vacancy vacancy:vacanciesDto.getItems()){
            isContainsText=isContains(vacancy.name,findText.substring(1,findText.length()-1))
                    ||isContains(vacancy.snippet.requirement,findText.substring(1,findText.length()-1))
                    ||isContains(vacancy.snippet.responsibility,findText.substring(1,findText.length()-1));
            if(!isContainsText)
                break;
        }
        assertTrue(isContainsText);
    ...
    public void testFindVacancyWithBooleanParameter() {
        ...
        System.out.println("Поиск по всем полям вакансий, содержащих слово " + findText.toUpperCase()
